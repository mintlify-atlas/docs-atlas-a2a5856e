...
When a purchase is confirmed (`POST /inventario/compras`), the backend iterates every `DetalleCompraInput` and adds `cantidad` to the corresponding product's `stock_actual`. No manual stock adjustment is needed.
...
| Edit product *(Admin)* | `/inventario/productos/editar/:id` |
