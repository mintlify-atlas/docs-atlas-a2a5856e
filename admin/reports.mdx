---
title: "Generate PDF Reports: Sales, Cash, Inventory, and Productivity"
...
The generated PDF uses the `CajaHistorialPdf` component structure — an A4 landscape-friendly document with a totals summary box that breaks down ingresos, egresos, and net cash flow:

```typescript
// src/components/pdf/CajaHistorialPdf.tsx (totals shape)
...
export const TicketVentaPdf: React.FC<{ data: VentaData }> = ({ data }) => (
  <Document>
    <Page size="A5" style={styles.page}>
      {/* Header: TICKET DE VENTA / Kantuta POS */}
      {/* Info: Nro. Ticket, Fecha, Cajero, Método Pago, Estado */}
      {/* Line items table: Producto / Cant. / P. Unit / Subtotal */}
      {/* Footer: TOTAL: Bs. {total} */}
    </Page>
  </Document>
);
```
